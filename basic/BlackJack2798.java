...
public class BlackJack2798 {
    public static void main(String[] args) {
        ...
        // 카드에 쓰인 숫자들을 저장할 배열을 선언합니다.
        int[] card = new int[N];
        // 카드에 쓰인 숫자들을 입력받아 배열에 저장합니다.
        for (int i = 0; i < N; i++) {
            card[i] = scanner.nextInt();
        }
        // 결과값을 저장할 변수를 선언하고, 0으로 초기화합니다.
        int result = 0;
        // 모든 카드 3장의 조합을 찾아보며, 조건에 맞는 최대값을 찾습니다.
        for (int i = 0; i < N - 2; i++) {
            for (int j = i + 1; j < N - 1; j++) {
                for (int k = j + 1; k < N; k++) {
                    int sum = card[i] + card[j] + card[k];

                    // 합이 M을 넘지 않으면서, 이전에 찾은 결과보다 큰 경우 결과를 갱신합니다.
                    if (sum <= M && result < sum) {
                        result = sum;
                    }
                }
